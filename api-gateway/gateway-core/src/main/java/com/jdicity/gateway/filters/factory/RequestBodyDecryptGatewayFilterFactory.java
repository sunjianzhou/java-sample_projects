...
import com.jdicity.gateway.filters.function.RequestBodyDecrypt;
 ...
 */
@Component
public class RequestBodyDecryptGatewayFilterFactory extends AbstractGatewayFilterFactory<FactoryConfig> {
    @Resource
    RequestBodyDecrypt requestBodyDecrypt;

    private final List<HttpMessageReader<?>> messageReaders;

    public RequestBodyDecryptGatewayFilterFactory() {
    ...
    public GatewayFilter apply(FactoryConfig config) {
        ...
        config.setRewriteFunction(requestBodyDecrypt);

        return new RequestBodyDecryptGatewayFilterFactory.ModifyRequestGatewayFilter(config);
    }
    ...
    public class ModifyRequestGatewayFilter implements GatewayFilter, Ordered {
        ...
        public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
            ...
            ServerRequest serverRequest = ServerRequest.create(exchange, RequestBodyDecryptGatewayFilterFactory.this.messageReaders);
        ...
        @Override
        public int getOrder() {
            return FilterOrderEnum.REQUEST_BODY_DECRYPT.getOrder();
        }
    }
}
